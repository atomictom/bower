...
GitRemoteResolver.refs = function (source) {
    ...
    // The total time until the retry will give up is ~68 minutes (which comes
    // from the sum of all exponential backoffs: 2^12 - 1 seconds). There
    // appears to be throttling in Github for ls-remote of 10 per hour.
    var retryOptions = {
        retries: 12,
        factor: 2,
        minTimeout: 1000,
    ...
    value = cmd('git', ['ls-remote', '--tags', '--heads', source], undefined, retryOptions)
