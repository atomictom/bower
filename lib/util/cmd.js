...
// Consider specifying retryOptions if your command is idempotent. See the
// documentation for promise-retry for details.
function cmd(command, args, options, retryOptions) {
    var enqueuedCmd = throttler.enqueue(executeCmd.bind(null, command, args, options));

    if (retryOptions == undefined) {
        return enqueuedCmd;
    }

    return Q(promiseRetry(retryOptions, function (retry, number) {
        return enqueuedCmd.catch(function (err) {
            retry(err);
        });
    }));
}
